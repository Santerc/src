 ...
 *  1. about line 323 change judger(originally sqrt(x^2+y^2) and yaw) into x and y
 *  2. about line 650 change State Matrix elements(originally x^2+y^2 and yaw) into x and y
 *********************************************************************/
                                                     ...
                                                     geometry_msgs::TwistStamped& cmd_vel, std::string& message)
{
    ...
    //if (std::abs(std::sqrt(dx * dx + dy * dy)) < _params.xy_goal_tolerance && std::abs(delta_orient) < _params.yaw_goal_tolerance)        //Liu Zeyuan: 这里判断是否到达目标位置，他的想法是判断要走的距离以及姿态角是否达到，现将其改为x以及y分别到了与否
    if (dx < _params.xy_goal_tolerance && dy < _params.xy_goal_tolerance)
    {
        _goal_reached = true;
    ...
    // Overwrite goal orientation if needed                                                //Liu Zeyuan: 哨兵不需要的
...

    {
        ...
        success = _controller.step(transformed_plan, _robot_vel, dt, t, _u_seq, _x_seq);     //Liu Zeyuan: _u_seq 外部输入 _x_seq 系统状态
    }
    ...
    bool feasible = _controller.isPoseTrajectoryFeasible(_costmap_model.get(), _footprint_spec, _robot_inscribed_radius, _robot_circumscribed_radius,
                                                         _params.collision_check_min_resolution_angular, _params.collision_check_no_poses);                //Liu Zeyuan: 不确定要改与否，再议
    if (!feasible)
    {
        cmd_vel.twist.linear.x = cmd_vel.twist.linear.y = cmd_vel.twist.angular.z = 0;
...
void MpcLocalPlannerROS::updateObstacleContainerWithCostmapConverter()                                         //Liu Zeyuan: 根据数据在地图上建立不同类型的障碍物
{
...
// Eigen::Vector2d MpcLocalPlannerROS::tfPoseToEigenVector2dTransRot(const tf::Pose& tf_vel)                                                                 //Liu Zeyuan: 状态矩阵的写入 第一个为间距，第二个为YAW角，现将其修改为第一个为x,第二个为y
// {
//     Eigen::Vector2d vel;
...
{
    Eigen::Vector2d vel;
    vel.coeffRef(0) = tf_vel.getOrigin().getX();
    vel.coeffRef(1) = tf_vel.getOrigin().getY();
    return vel;
}
...
}                                                                                                                                                  //Liu Zeyuan: 本函数计算为角度，为哨兵所不需要的,目前判断无大关系，暂时不去进一步深究
