 ...
 *  1. about line 82 change output of goal (originally distance & angle) to dist_x & dist_y
 *  2. about line 162 change judger from dist and angle to dist
 *  3. change model simple car into Sentry mode
 *********************************************************************/
                           ...
                           teb_local_planner::RobotFootprintModelPtr robot_model, const std::vector<teb_local_planner::PoseSE2>& via_points)
{
    ...
    nh.param("controller/force_reinit_new_goal_dist_x", _force_reinit_new_goal_dist_x, _force_reinit_new_goal_dist_x);                  //Liu Zeyuan; 此处为目标位置输出，将极坐标系改为直角坐标系
    nh.param("controller/force_reinit_new_goal_dist_y", _force_reinit_new_goal_dist_y, _force_reinit_new_goal_dist_y);
                      ...
                      corbo::TimeSeries::Ptr u_seq, corbo::TimeSeries::Ptr x_seq)
{
    ...
    if (!_grid->isEmpty() && ((goal.position() - _last_goal.position()).norm() > _force_reinit_new_goal_dist_x ||
                               (goal.position() - _last_goal.position()).norm() > _force_reinit_new_goal_dist_y))                          //Liu Zeyuan: 修改了判据：只要求位置达到
    {
                                                                     ...
                                                                     const std::vector<teb_local_planner::PoseSE2>& via_points)
{
    ...
    else if (_robot_type == "simple_car")
    {
        ...
        nh.param("robot/simple_car/max_vel_y", max_vel_y, max_vel_y);
        double max_vel_y_backwards = 0.4;
        nh.param("robot/simple_car/max_vel_y_backwards", max_vel_y_backwards, max_vel_y_backwards);
        if (max_vel_y_backwards < 0)
        {
            ROS_WARN("max_vel_y_backwards must be >= 0");
            max_vel_y_backwards *= -1;
        }
        ocp->setControlBounds(Eigen::Vector2d(-max_vel_x_backwards, -max_vel_y_backwards), Eigen::Vector2d(max_vel_x, max_vel_y));
    }
    else if (_robot_type == "kinematic_bicycle_vel_input")
    {
        double max_vel_x = 0.4;
